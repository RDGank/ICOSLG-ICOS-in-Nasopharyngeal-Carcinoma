...
DotPlot(sce,features = c(CD8_markers_list1),assay = 'RNA')



library(stringr)
sce$celltype<-sce$seurat_clusters
sce$celltype<-str_replace(sce$celltype,'12','T_Memory')
...
sce$celltype<-str_replace(sce$celltype,'10','NK')
DimPlot(sce,group.by = 'celltype')
sce$celltype<-str_replace(sce$celltype,'4|5','T_Naive')
sce$celltype<-str_replace(sce$celltype,'','')
sce$celltype<-str_replace(sce$celltype,'0','Treg')
sce$celltype<-str_replace(sce$celltype,'6','Tfh')
sce$celltype<-str_replace(sce$celltype,'','T_Memory')
sce$celltype<-str_replace(sce$celltype,'1|2|3|7|9','T_Memory')
sce$celltype<-str_replace(sce$celltype,'8','Tex')
sce$celltype<-str_replace(sce$celltype,'','')
sce$celltype<-str_replace(sce$celltype,'','')
sce$celltype<-str_replace(sce$celltype,'','')
sce$celltype<-str_replace(sce$celltype,'','')
...
save(sce,file = 'sce_GSE162025_Tcell.RData')
