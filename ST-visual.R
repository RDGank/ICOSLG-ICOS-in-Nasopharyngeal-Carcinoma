T7<-ST5
p1<-SpatialPlot(T7, label = TRUE, label.size = 5,pt.size.factor = 4)
p2 <- DimPlot(T7, reduction = "umap", label = TRUE)
p1+p2
p3<-SpatialFeaturePlot(T7,features = c('EPCAM','TUBB2B','ICOSL','ICOS','FOXP3'),pt.size.factor = 6,ncol = 4)
p3
#p3<-
...
T7<-AddModuleScore(T7,features = as.list(genes),name = 'Col-NSCLC(ITGA2)_Mac(SPP1)')
T7<-AddModuleScore(T7,features = as.list(genes1),name = 'Abundance_Mac-SPP')
T7<-AddModuleScore(T7,features = as.list(genes2),name = 'Abundance_NSCLC(ITGA2)_')
p4<-SpatialFeaturePlot(T7,features = c('Col-NSCLC(ITGA2)_Mac(SPP1)1'),pt.size.factor = 6)

metadata<-T7@meta.data
...
px
p0<-SpatialFeaturePlot(T7, features = "nFeature_Spatial",pt.size.factor = 4)
px<-p0+p1+p4+plot_layout(design = 'ABC')
py<-(p2+p3)+plot_layout(design = 'ABBBB')
pdf(file = 'col-Mal_macro-ST.pdf',height = 6,width = 12)
px/py
dev.off()
